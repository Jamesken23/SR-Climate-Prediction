...
from Lib.model_methods import train_model
...
def set_patams(mode, area):
    ...
    try:
        Epoch = int(tool.parse_config_test(config_path, "common_option", "Epoch"))
        learning_rate = float(tool.parse_config_test(config_path, "common_option", "learning_rate"))
        batch_size = int(tool.parse_config_test(config_path, "common_option", "batch_size"))
        split = float(tool.parse_config_test(config_path, "common_option", "split"))
        ...
        input_size_w = int(tool.parse_config_test(config_path, area, "input_size_w"))
        input_size_j = int(tool.parse_config_test(config_path, area, "input_size_j"))
        output_size_w = int(tool.parse_config_test(config_path, area, "output_size_w"))
        output_size_j = int(tool.parse_config_test(config_path, area, "output_size_j"))
    ...
    checkpoint = os.path.join(os.getcwd(), "weights-10")
    ...
    batch_size = 4
    return Data_set(Epoch, learning_rate, batch_size, train_path, val_path, dem_path, checkpoint, input_size_w,
                    input_size_j, output_size_w, output_size_j, split, True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        exit()
    args = set_patams(mode, area)
    scale = args.output_size_w // args.input_size_w + 1
    model = Net(num_channels=3, base_filter=16, num_residuals=10, upscale_factor=scale, output_size_w=args.output_size_w,
                output_size_j=args.output_size_j)
    args.model = model
    args.model_name = "VDSR"
    # 训练模型
    train_model(args)
