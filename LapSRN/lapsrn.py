...
class Net(nn.Module):
    def __init__(self, num_channels=3, base_filter=16, num_residuals=10, input_size_w=1, input_size_j=1,
        ...
        self.relu = nn.LeakyReLU(0.2, inplace=True)
        # 对输入图片做反卷积
        self.convt_I1 = nn.ConvTranspose2d(in_channels=num_channels, out_channels=1, kernel_size=4, stride=self.first_scale, padding=1,
                                           bias=False)
        ...
        self.convt_F2 = self.make_layer(Conv_ReLU_Block(base_filter, base_filter), num_residuals)
    ...
    def forward(self, x):
        ...
        # HR_2x = torch.from_numpy(HR_2x_numpy)
