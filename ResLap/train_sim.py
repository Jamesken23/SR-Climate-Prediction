...
from ResLap.reslap_sim import Net, L2_Charbonnier_loss
import os
import torch
from Lib.model_methods import train_multi_models, train_model
...
class Data_set():
    def __init__(self, Epoch, lr, batch_size, train_path, val_path, dem_path, ckpt, input_size_w, input_size_j,
                 mid_size_w, mid_size_j, output_size_w, output_size_j, split, is_training):
...
if __name__ == "__main__":
    ...
    # 定义Multi_L2损失函数
    model.loss_func = torch.nn.SmoothL1Loss()
    batch_size = 4
    args = Data_set(Epoch, learning_rate, batch_size, train_path, val_path, dem_path, checkpoint, input_size_w, input_size_j,
                    mid_size_w, mid_size_j,output_size_w, output_size_j, split, True)
    args.model = model
    args.model_name = "ResLap"
    # print("train_data_dir", train_data_dir)
    # print("dem_path", dem_path)
    # 训练模型
    train_model(args)
    # train_multi_models(args)